...
class DatagridHelper {
    ...
    public function __construct($config, $doctrine, $logger) {
    	$this->config = $config;
    ...
    private function getNumber() {
        ...
        $query->useResultCache($this->getDatagrid()->getCacheEnabled(), $this->getDatagrid()->getCacheSeconds(), $this->getDatagrid()->getCachePrefix().'_scalar');
    ...
    private function getResults() {
    	...
    	if ($this->qb) {
	    	...
	    	$query->useResultCache($this->getDatagrid()->getCacheEnabled(), $this->getDatagrid()->getCacheSeconds(), $this->getDatagrid()->getCachePrefix().'_result');
    ...
    /**
     * Returns the calculated pagination
     *
     * @return \HBM\CyberclubBundle\Model\Pagination
     */
    public function paginate() {
    	...
    	$pagination->setPageCurrent(max(array(1, min(array($pagination->getPageCurrent(), $pagination->getPageMax())))));

    	$pagination->setOffset(max(array(0, ($pagination->getPageCurrent() - 1) * $max_entries)));

    	// Calculate numbers
    	$pagination->setNumberFrom(0);
    	$pagination->setNumberThru(0);
    	if ($pagination->getNumberTotal() > 0) {
    		$pagination->setNumberFrom(max(array(1, ($pagination->getPageCurrent() - 1) * $max_entries + 1)));
    		$pagination->setNumberThru(min(array($pagination->getNumberTotal(), $pagination->getNumberFrom() + $max_entries - 1)));
    	}

    	// Calculate pages
    	$pagination->setPageFrom(max(array(1, ($pagination->getPageCurrent() - floor(($max_links - 1) / 2)))));
    	$pagination->setPageThru(min(array($pagination->getPageMax(), ($pagination->getPageFrom() + $max_links - 1))));
    	if ($pagination->getPageThru() == $pagination->getPageMax()) {
    		$pagination->setPageFrom(max(array(1, ($pagination->getPageThru() - $max_links))));
    	}
